...
from trivena_insights.tests.factories import DT, create_user, delete_users, delete_workbooks
...
TEST_DS = trivena.scrub(TEST_DS_TITLE)
...
def delete_test_data_sources():
    trivena.delete_doc(DT.DATA_SOURCE, TEST_DS, force=True)
...
def delete_test_tables():
    trivena.delete_doc(DT.TABLE, TEST_TABLE1, force=True)
    trivena.delete_doc(DT.TABLE, TEST_TABLE2_NAME, force=True)
    trivena.delete_doc(DT.TABLE, TEST_TABLE3_NAME, force=True)


def create_test_teams():
    team = trivena.get_doc({"doctype": DT.TEAM, "team_name": "team1"})
...
def delete_test_teams():
    trivena.delete_doc(DT.TEAM, "team1", force=True)


def update_dashboard_access(dashboard_name, people_with_access):
    trivena.get_doc(DT.DASHBOARD, dashboard_name).update_access(
        {
            "is_public": 0,
...
def share_chart(chart_name, user):
    trivena.share.add(DT.CHART, chart_name, user=user, read=1, notify=0)


def unshare_chart(chart_name, user):
    share_name = trivena.db.get_value(
        "DocShare",
        {
    ...
    if share_name:
        trivena.delete_doc("DocShare", share_name, ignore_permissions=True)
