...
from trivena_insights.api.workbooks import (
    create_folder,
    delete_folder,
...
from trivena_insights.tests.factories import DT, USER_1, create_test_user, delete_users
from trivena_insights.tests.workbook_utils import (
...
class TestWorkbook(InsightsIntegrationTestCase):
    ...
    def test_owner_can_build_workbook_query_chart_dashboard_flow(self):
        ...
        dashboard_items = trivena.parse_json(dashboard["items"]) or []
        ...
        self.assertTrue(trivena.db.exists(DT.QUERY, chart["data_query"]))
    ...
    def test_deleting_workbook_removes_the_user_visible_tree(self):
        ...
        with self.as_user("Administrator"):
            trivena.delete_doc(DT.WORKBOOK, bundle["workbook"].name, force=True)

        self.assertFalse(trivena.db.exists(DT.WORKBOOK, bundle["workbook"].name))
        self.assertFalse(trivena.db.exists(DT.QUERY, bundle["query"].name))
        self.assertFalse(trivena.db.exists(DT.CHART, bundle["chart"].name))
        self.assertFalse(trivena.db.exists(DT.QUERY, data_query_name))
        self.assertFalse(trivena.db.exists(DT.DASHBOARD, bundle["dashboard"].name))
    ...
    def test_duplicate_workbook_preserves_a_usable_copy(self):
        ...
        duplicate_dashboard_items = trivena.parse_json(duplicate_dashboard["items"]) or []
        with self.as_user(USER_1):
    ...
    def test_export_and_import_preserve_a_usable_workflow(self):
        ...
        imported_dashboard_items = trivena.parse_json(imported_dashboard["items"]) or []
        with self.as_user(USER_1):
