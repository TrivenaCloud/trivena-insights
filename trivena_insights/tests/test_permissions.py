...
from trivena_framework.permissions import update_permission_property

from trivena_insights.api.workbooks import get_share_permissions, update_share_permissions
...
from trivena_insights.insights.doctype.insights_team.insights_team import clear_cache as clear_team_cache
from trivena_insights.permissions import PERMISSION_DOCTYPES
from trivena_insights.tests.base import InsightsIntegrationTestCase
from trivena_insights.tests.factories import (
    DT,
    create_test_chart,
...
from trivena_insights.tests.permissions_utils import (
    ADMIN,
    NON_INSIGHTS_USER,
    TEST_DS,
    TEST_TABLE1,
...
class TestInsightsPermissions(InsightsIntegrationTestCase):
    ...
    def before_class(cls):
        cls.original_enable_permissions = trivena.db.get_single_value(DT.SETTINGS, "enable_permissions")
        cleanup_test_fixtures()
        create_test_users()
        clear_team_cache()
    ...
    @classmethod
    def after_class(cls):
        trivena.db.set_single_value(DT.SETTINGS, "enable_permissions", cls.original_enable_permissions)
        clear_team_cache()
        cleanup_test_fixtures()
    ...
    def toggle_team_permissions(self, enable):
        trivena.db.set_single_value(DT.SETTINGS, "enable_permissions", enable)
        clear_team_cache()
    ...
    def test_permissions_for_non_insights_user(self):
        with self.as_user(NON_INSIGHTS_USER):
            for doctype in PERMISSION_DOCTYPES:
                self.assertFalse(
                    trivena.has_permission(doctype, ptype="read"),
    ...
    def test_permission_for_dashboard(self):
        ...
        with self.as_user(USER_2):
            self.assertFalse(trivena.has_permission(DT.DASHBOARD, ptype="write", doc=dashboard.name))
            with self.assertRaises(trivena.PermissionError):
                create_test_dashboard(
                    USER_2,
                    workbook.name,
    ...
    def test_permission_for_chart(self):
        ...
        self.assert_visible_to(USER_2, DT.CHART, chart.name)
        with self.as_user(USER_2):
            self.assertFalse(trivena.has_permission(DT.CHART, ptype="write", doc=chart.name))
            with self.assertRaises(trivena.PermissionError):
                create_test_chart(
                    USER_2,
                    workbook.name,
    ...
    def test_permission_for_query(self):
        ...
        chart = trivena.get_doc(DT.CHART, chart.name)
    ...
    def test_download_results_requires_export_permission(self):
        ...
        query = create_test_query(USER_1, workbook.name)
        self.toggle_team_permissions(False)
        trivena.db.set_single_value(DT.SETTINGS, "allow_download", 1)
        self.addCleanup(trivena.clear_cache, doctype=DT.QUERY)

        update_permission_property(DT.QUERY, "Insights User", 0, "export", 0)
        trivena.clear_cache(doctype=DT.QUERY)

        with self.as_user(USER_1):
            query_doc = trivena.get_doc(DT.QUERY, query.name)
            with self.assertRaisesRegex(trivena.PermissionError, "export permission"):
    ...
    def test_download_results_requires_document_access(self):
        ...
        with self.as_user(USER_2):
            self.assertTrue(trivena.has_permission(DT.QUERY, ptype="export"))
            query_doc = trivena.get_doc(DT.QUERY, query.name)
            with self.assertRaises(trivena.PermissionError):
                query_doc.download_results(format="csv")
    ...
    def test_download_results_allowed_with_read_only_share(self):
        ...
        with self.as_user(USER_2):
            self.assertTrue(trivena.has_permission(DT.QUERY, ptype="export"))
            self.assertFalse(trivena.has_permission(DT.QUERY, ptype="write", doc=query.name))
            query_doc = trivena.get_doc(DT.QUERY, query.name)
            with db_connections():
                csv_data = query_doc.download_results(format="csv")
            self.assertIsInstance(csv_data, str)
    ...
    def test_download_results_blocked_when_globally_disabled(self):
        ...
        query = create_test_query(USER_1, workbook.name)
        self.toggle_team_permissions(False)
        trivena.db.set_single_value(DT.SETTINGS, "allow_download", 0)
        ...
        with self.as_user(USER_1):
            self.assertTrue(trivena.has_permission(DT.QUERY, ptype="export"))
            query_doc = trivena.get_doc(DT.QUERY, query.name)
            with self.assertRaisesRegex(trivena.PermissionError, "not allowed to download"):
                query_doc.download_results(format="csv")
