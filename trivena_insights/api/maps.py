...
def get_map_json_path(map_type: str) -> str:
    ...
    if map_type not in ("india", "world"):
        trivena.throw("Invalid map type")

    filename = "india.json" if map_type == "india" else "world_map.json"
    return os.path.join(trivena.get_app_path("trivena_insights"), "public", "maps", filename)
...
def extract_regions_from_geojson(map_type: str) -> list:
    ...
    if not os.path.exists(file_path):
        trivena.throw(f"Map file not found: {file_path}")
...
def get_chart_config(chart_name: str) -> dict:
    ...
    if not trivena.db.exists("Insights Chart v3", chart_name):
        return {}

    chart = trivena.get_doc("Insights Chart v3", chart_name)
    config = chart.config or {}

    if isinstance(config, str):
        config = trivena.parse_json(config) or {}
    return config
...
def save_chart_config(chart_name: str, config: dict):
    """Save chart config"""
    if not trivena.has_permission("Insights Chart v3", "write", chart_name):
        trivena.throw("Not permitted to modify this chart", trivena.PermissionError)
    chart = trivena.get_doc("Insights Chart v3", chart_name)
    chart.config = config
    chart.save()
...
def find_unresolved_regions(map_type: str, user_regions: list, chart_name: str) -> dict:
    ...
    if chart_name and not trivena.has_permission("Insights Chart v3", ptype="read", doc=chart_name):
        trivena.throw("You do not have access to this chart", trivena.PermissionError)
