...
from trivena_framework.utils import cint
from trivena_framework.utils.synchronization import filelock

from trivena_insights.decorators import insights_whitelist
from trivena_insights.utils import DocShare, deep_convert_dict_to_dict
...
def get_installed_apps() -> set[str]:
    # narrow seam over trivena.get_installed_apps so tests can fake the app set
    # without patching the global that frappe's own insert hooks rely on
    return set(trivena.get_installed_apps())
...
def _app_title(app: str) -> str:
    ...
    try:
        return (trivena.get_hooks("app_title", app_name=app) or [app])[0]
    except Exception:
        return app
...
def _discover_templates() -> dict[str, dict]:
    ...
    for app in sorted(trivena.get_installed_apps()):
        for rel_path in trivena.get_hooks(TEMPLATES_HOOK, app_name=app) or []:
            base = os.path.join(trivena.get_app_path(app), rel_path)
...
def _resolve_template(template_name: str) -> dict:
    ...
    if not entry:
        trivena.throw(_("Workbook template {0} does not exist").format(trivena.bold(template_name)))
    return entry
...
def has_source_data(manifest: dict) -> bool:
    ...
    for doctype in manifest.get("source_doctypes") or []:
        if not trivena.db.table_exists(doctype):
            continue
        table = trivena.qb.DocType(doctype)
        if trivena.qb.from_(table).select(table.name).limit(1).run():
            return True
    return False
...
def get_workbook_templates() -> list[dict]:
    ...
    for name, entry in sorted(_discover_templates().items()):
        ...
        imported_version = (
            cint(trivena.db.get_value("Insights Workbook", imported_workbook, "imported_version"))
...
def _reassign_to_administrator(workbook_name: str) -> None:
    ...
    trivena.db.set_value("Insights Workbook", workbook_name, "owner", "Administrator")
    for doctype in _WORKBOOK_CHILD_DOCTYPES:
        trivena.db.set_value(doctype, {"workbook": workbook_name}, "owner", "Administrator")
...
def _template_import_result(workbook_name: str) -> dict:
    ...
    first_dashboard = trivena.db.get_value(
        "Insights Dashboard v3",
...
def create_workbook_from_template(template_name: str) -> dict:
    ...
    if not has_required_apps(manifest):
        trivena.throw(
            _("Workbook template {0} requires apps that are not installed: {1}").format(
                trivena.bold(manifest.get("title") or template_name),
    ...
    with filelock(lock_key, timeout=60):
        ...
        # Import as the caller (don't trivena.set_user mid-request — it rewrites the
        ...
        trivena.db.set_value("Insights Workbook", workbook_name, "from_template", template_name)
        ...
        trivena.db.commit()  # nosemgrep — intentional commit inside the import lock (see above)
...
def _workbook_checksum(workbook_name: str) -> str:
    ...
    payload = trivena.get_doc("Insights Workbook", workbook_name).export()
...
def _is_customized(workbook_name: str) -> bool:
    ...
    stored = trivena.db.get_value("Insights Workbook", workbook_name, "imported_checksum")
...
def _replace_workbook_contents(workbook_name: str, workbook_json: dict) -> None:
    ...
    workbook = deep_convert_dict_to_dict(trivena.parse_json(workbook_json))
    for doctype in _WORKBOOK_CHILD_DOCTYPES:
        for row in trivena.get_all(doctype, {"workbook": workbook_name}):
            trivena.delete_doc(doctype, row.name, force=True, ignore_permissions=True)

    doc = trivena.get_doc("Insights Workbook", workbook_name)
...
def update_workbook_from_template(template_name: str) -> dict:
    ...
    with filelock(lock_key, timeout=60):
        ...
        trivena.db.commit()  # nosemgrep — intentional commit inside the import lock
...
def sync_workbook_template_updates() -> None:
    ...
    trivena.db.commit()  # nosemgrep — flush prior after_migrate work before per-copy rollbacks
    for template_name, workbook_name in get_imported_templates().items():
        ...
        imported_version = cint(trivena.db.get_value("Insights Workbook", workbook_name, "imported_version"))
        try:
            if not imported_version:
                ...
                trivena.db.set_value("Insights Workbook", workbook_name, "imported_version", version)
            ...
            trivena.db.commit()  # nosemgrep — land each copy atomically (paired with the rollback below)
        except Exception:
            trivena.db.rollback()
            trivena.log_error(title=f"Failed to sync workbook template {template_name}")
