...
from trivena_framework.query_builder.functions import Count, Max
...
def get_dashboards(
    ...
    filters = {}
    if get_favorites:
        filters["_liked_by"] = ["like", f"%{trivena.session.user}%"]
...
def get_recent_dashboards(search_term: str | None = None, limit: int = 20):
    ...
    recent = (
        ...
        .orderby(Max(view_log.modified), order=trivena.qb.desc)
        .limit(limit)
        .run(as_dict=True)
    )
...
def _enrich_dashboards(dashboards):
    ...
    for dashboard in dashboards:
        ...
        if dashboard._liked_by:
            dashboard["is_favourite"] = user in trivena.as_json(dashboard._liked_by)
...
def _dashboard_chart_counts(names: list[str]) -> dict[str, int]:
    ...
    if not names:
        return {}
    linked_chart = trivena.qb.DocType("Insights Dashboard Chart v3")
    rows = (
        trivena.qb.from_(linked_chart)
...
def update_dashboard_preview(dashboard_name: str):
    trivena.has_permission("Insights Dashboard v3", ptype="read", doc=dashboard_name, throw=True)
    dashboard = trivena.get_doc("Insights Dashboard v3", dashboard_name)
