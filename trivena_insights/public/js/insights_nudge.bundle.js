...
(function () {
	...
	const BASE =
		(trivena.boot.app_data || []).find((a) => a.app_name === "trivena_insights")
			?.app_route || "/insights";
	...
	const allowed = () =>
		ALLOWED_ROLES.some((r) => (trivena.user_roles || []).includes(r));
	// Frappe gates its own CRM/Helpdesk workspace nudges on the same flag.
	const suggestionsDisabled = () =>
		cint(trivena.sys_defaults?.disable_product_suggestion);
	...
	const dismissKey = (cfg) =>
		`insights:nudge:${trivena.session.user}:${templateId(cfg)}`;
	const dismissed = (cfg) => localStorage.getItem(dismissKey(cfg)) === "1";
	const esc = (s) => trivena.utils.escape_html(s);

	// no-ops when telemetry is off
	const track = (event, ws, cfg) =>
		trivena.telemetry?.capture(event, "trivena_insights", {
			workspace: ws,
			template: templateId(cfg),
		});

	function currentWorkspace() {
		const route = trivena.get_route() || [];
	...
	function init() {
		if (!window.frappe || !trivena.router || !trivena.get_route) {
			return setTimeout(init, 300); // self-heal against load order
		}
		injectStyles();
		trivena.router.on("change", () => tryShow());
		tryShow();
	}
