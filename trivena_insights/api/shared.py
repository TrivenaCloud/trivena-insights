...
from trivena_framework.query_builder import DocType

from trivena_insights.decorators import validate_type
...
def is_public_workbook(name: str):
    public_dashboard_exists = trivena.db.exists(
        "Insights Dashboard v3",
        {
            "workbook": name,
            "is_public": 1,
...
def has_valid_preview_key():
    ...
    preview_key = trivena.request.headers.get("X-Insights-Preview-Key")
    return preview_key and trivena.cache.get_value(f"insights_preview_key:{preview_key}")
...
def get_public_charts():
    ...
    public_dashboards = trivena.qb.from_(Dashboard).select(Dashboard.name).where(Dashboard.is_public == 1)
    ...
    charts = (
        ...
        .where(
            ...
            | (
                Chart.name.isin(
                    trivena.qb.from_(DashboardChart)
...
def get_dashboard_name(dashboard_name: str):
    name = dashboard_name
    if not trivena.db.exists("Insights Dashboard v3", name):
        new_name = trivena.db.exists("Insights Dashboard v3", {"old_name": name})
        if new_name:
            name = new_name
    return name
...
def get_chart_name(chart_name: str):
    name = chart_name
    if not trivena.db.exists("Insights Chart v3", name):
        new_name = trivena.db.exists("Insights Chart v3", {"old_name": name})
        if new_name:
            name = new_name
    return name
