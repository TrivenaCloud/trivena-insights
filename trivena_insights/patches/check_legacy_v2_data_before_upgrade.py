...
def execute():
    if not trivena.db.count("Insights Query"):
        return

    if trivena.conf.get("insights_allow_legacy_v2_upgrade"):
        return

    log_count = trivena.db.count(
        "Insights Query Execution Log",
        filters={
            "creation": [">=", trivena.utils.add_to_date(trivena.utils.now(), days=-30)],
            "data_source": ["is", "set"],
            "query": ["is", "set"],
        },
    )
...
You can check the migration guide here: https://docs.trivena.io/insights/articles/migrate-from-v2-to-v3

For any questions or help with the migration, please join telegram group: https://t.me/frappeinsights or reach out to support: https://trivena.io/support
