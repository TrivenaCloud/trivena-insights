...
from trivena_insights.api.data_sources import get_data_source_tables
...
class DemoDataFactory:
    ...
    def initialize(self):
        ...
        self.files_folder = trivena.get_site_path("private", "files")
    ...
    def setup_demo_data_source(self):
        if not trivena.db.exists("Insights Data Source v3", "demo_data"):
            data_source = trivena.new_doc("Insights Data Source v3")
        ...
        self.data_source = trivena.get_doc("Insights Data Source v3", "demo_data")
    ...
    def demo_data_exists(self):
        ...
        links_count = trivena.db.count("Insights Table Link v3", {"data_source": self.data_source.name})
    ...
    def download_demo_data(self):
        ...
        try:
            ...
            trivena.log_error("Error downloading demo data. Please check your Internet connection.")
    ...
    def create_table_links(self):
        ...
        trivena.db.delete("Insights Table Link v3", {"data_source": self.data_source.name})
    ...
    def create_sample_workbook(self):
        if trivena.db.exists("Insights Workbook", {"title": "Order Analysis"}):
            return

        fixture_path = trivena.get_app_path("trivena_insights", "setup")
        with open(fixture_path + "/sample_workbook.json") as f:
            sample_workbook = f.read()

        import_workbook(trivena.parse_json(sample_workbook))
