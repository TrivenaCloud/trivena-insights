...
def check_role(role):
    def decorator(function):
        ...
        def wrapper(*args, **kwargs):
            ...
            has_required_role = role in trivena.get_roles(trivena.session.user)
            if role == "Insights User" and is_admin(trivena.session.user):
                has_required_role = True

            if not has_required_role:
                trivena.throw(
                    trivena._("You do not have permission to access this resource"),
...
def check_permission(doctype, permission_type="read"):
    def decorator(function):
        @wraps(function)
        def wrapper(*args, **kwargs):
            trivena.has_permission(doctype, permission_type, throw=True)
            return function(*args, **kwargs)
    ...
    def decorator(function):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                trivena.log_error("Insights Error")
