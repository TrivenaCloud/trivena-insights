...
import pandas as pd
from trivena_framework.utils import now_datetime
from ibis import _

from trivena_insights.insights.query_builders.sql_functions import (
...
def count(
    ...
    if column is None:
        query = trivena.flags.current_ibis_query
...
def create_buckets(column: ir.Column, num_buckets: int):
    ...
    query = trivena.flags.current_ibis_query
    if query is None:
        trivena.throw("Failed to create buckets. Query not found")
    ...
    if not values:
        trivena.throw("Failed to create buckets. No data found in the column")
...
def fiscal_year_start(column: ir.DateValue):
    ...
    fy_start = trivena.db.get_single_value("Insights Settings", "fiscal_year_start")
    if not fy_start:
...
def get_retention_data(date_column: ir.DateValue, id_column: ir.Column, unit: str):
    ...
    if not date_column.type().is_date():
        trivena.throw(f"Invalid date column. Expected date, got {date_column.type()}")
