...
def get_max_execution_time() -> int:
    return trivena.db.get_single_value("Insights Settings", "max_execution_time", cache=True) or (
...
class InsightsSettings(Document):
    ...
    def update_settings(self, settings: dict | str):
        settings = trivena.parse_json(settings)
    ...
    def is_subscribed(self):
        try:
            return 1 if trivena.conf.sk_insights else 0
        except Exception:
            return None
