...
from trivena_insights.insights.query_utils import (
...
class InsightsQueryv3(Document):
    ...
    if TYPE_CHECKING:
        ...
        from trivena_insights.insights.doctype.insights_query_variable.insights_query_variable import (
    ...
    def get_valid_dict(self, *args, **kwargs):
        if isinstance(self.operations, list):
            self.operations = trivena.as_json(self.operations)
    ...
    def on_trash(self):
        for alert in trivena.get_all("Insights Alert", filters={"query": self.name}, pluck="name"):
            trivena.delete_doc("Insights Alert", alert, force=True, ignore_permissions=True)

        # Remove all edges referencing or referenced by this query
        trivena.db.delete("Insights Query Reference", {"query": self.name})
        trivena.db.delete("Insights Query Reference", {"ref_query": self.name})
    ...
    def _validate_no_circular_dependency(self):
        ...
        operations = trivena.parse_json(self.operations) or []
        ...
        if cycle:
            path_str = " → ".join(
                f'"{trivena.db.get_value("Insights Query v3", q, "title") or q}"' for q in cycle
            )
            trivena.throw(
    ...
    def cleanup_empty_folder(self, folder_name):
        ...
        if folder_type == "query":
            has_items = trivena.db.exists("Insights Query v3", {"folder": folder_name})
        else:
            has_items = trivena.db.exists("Insights Chart v3", {"folder": folder_name})

        if not has_items:
            trivena.delete_doc("Insights Folder", folder_name, force=True, ignore_permissions=True)
    ...
    def build(self, active_operation_idx=None, use_live_connection=None):
        ...
        if ibis_query is None:
            trivena.throw("Failed to build query")
    ...
    def execute(
        ...
        with suppress(Exception):
            for op in trivena.parse_json(self.operations) or []:
    ...
    def download_results(
        ...
        from trivena_insights.insights.doctype.insights_team.insights_team import is_admin

        if not is_admin(trivena.session.user) and not trivena.db.get_single_value(
        ...
        if not is_admin(trivena.session.user) and not (
            trivena.has_permission(self.doctype, ptype="export")
            and trivena.has_permission(self.doctype, ptype="read", doc=self)
        ):
            trivena.throw(
                trivena._(
    ...
    def export(self):
        ...
        for q in linked_queries:
            ...
            if not trivena.has_permission("Insights Query v3", ptype="read", doc=q):
                trivena.throw(
                    trivena._("You do not have access to one of the linked queries"),
                    trivena.PermissionError,
                )
            exported_query = trivena.get_doc("Insights Query v3", q).export()
    ...
    @insights_whitelist()
    def duplicate(self):
        new_query = trivena.copy_doc(self)
    ...
    def refresh_stored_tables(self):
        ...
        if not source_tables:
            trivena.throw("No tables found in the query to import")
        ...
        for table in source_tables:
            ...
            if data_source and table_name:
                ...
                if trivena.db.exists("Insights Table v3", table_doc_name):
                    table_doc = trivena.get_doc("Insights Table v3", table_doc_name)
...
def import_query(query, workbook):
    query = trivena.parse_json(query)
    ...
    operations = trivena.parse_json(new_query.operations)
...
def set_adhoc_filters(filters):
    # If trivena.local.insights_adhoc_filters exists but is None, getattr returns None.
    # We must ensure it's a dict.
    current = getattr(trivena.local, "insights_adhoc_filters", None)
    trivena.local.insights_adhoc_filters = filters or current or {}
    yield
    trivena.local.insights_adhoc_filters = None
