...
from trivena_framework.utils import add_days, now_datetime

import trivena_insights
from trivena_insights.insights.doctype.insights_data_source_v3 import data_warehouse
from trivena_insights.insights.doctype.insights_data_source_v3.connectors.duckdb import open_local_duckdb
from trivena_insights.insights.doctype.insights_data_source_v3.data_warehouse import (
    compact_warehouse,
...
class TestDataStoreCleanup(InsightsIntegrationTestCase):
    def before_test(self):
        trivena.db.delete("Insights Query Execution Log")
        trivena.db.delete("Insights Query Reference")
        trivena.db.delete("Insights Table Import Log")
    ...
    def create_table(self, table_name, stored=1, sync_mode="Full"):
        ...
        if trivena.db.exists("Insights Table v3", name):
            trivena.delete_doc("Insights Table v3", name, force=True)
    ...
    def log_execution(self, query, days_ago):
        log = trivena.get_doc({"doctype": "Insights Query Execution Log", "query": query, "sql": "select 1"})
    ...
    def is_stored(self, table_name):
        return trivena.db.get_value("Insights Table v3", get_table_name(DATA_SOURCE, table_name), "stored")
    ...
    def test_prunes_table_whose_queries_are_stale(self):
        ...
        self.assertFalse(
            trivena.db.get_value("Insights Table v3", table.name, "last_synced_on"),
