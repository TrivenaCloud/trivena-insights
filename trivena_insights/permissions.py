...
import trivena_framework.share
...
class InsightsPermissions:
    def __init__(self, user=None):
        self.user = user or trivena.session.user
        self.user_teams = []
    ...
    def team_permissions_enabled(self):
        return trivena.db.get_single_value("Insights Settings", "enable_permissions")
    ...
    def get_permission_query_conditions(self, doctype: str) -> str:
        ...
        if doctype == "Insights Team":
            ...
            item_list = [trivena.db.escape(item) for item in self.user_teams]
    ...
    def has_doc_permission(self, doc, ptype):
        ...
        if is_new and hasattr(doc, "workbook") and doc.workbook:
            ...
            return (
                docs.where(trivena.qb.DocType("Insights Workbook").name == doc.workbook)
                .limit(1)
                .run(pluck="name")
            )
        ...
        return docs.where(trivena.qb.DocType(doc.doctype).name == doc.name).limit(1).run(pluck="name")
    ...
    def _build_source_permission_query(self, ptype):
        ...
        if not self.team_permissions_enabled:
            return trivena.qb.from_(trivena.qb.DocType("Insights Data Source v3")).select("name")

        # if team permissions are enabled, allow data sources of allowed tables
        DataSource = trivena.qb.DocType("Insights Data Source v3")
        Table = trivena.qb.DocType("Insights Table v3")
        AllowedTables = self._build_table_permission_query(ptype)

        return (
            trivena.qb.from_(DataSource)
    ...
    def _build_table_permission_query(self, ptype):
        ...
        if not self.team_permissions_enabled:
            return trivena.qb.from_(trivena.qb.DocType("Insights Table v3")).select("name")
    ...
    def _build_workbook_permission_query(self, ptype):
        ...
        Workbook = trivena.qb.DocType("Insights Workbook")

        OwnedWorkbooks = trivena.qb.from_(Workbook).select(Workbook.name).where(Workbook.owner == self.user)
    ...
    def _build_dashboard_permission_query(self, ptype):
        Dashboard = trivena.qb.DocType("Insights Dashboard v3")
        OwnedDashboards = (
            trivena.qb.from_(Dashboard).select(Dashboard.name).where(Dashboard.owner == self.user)
        )
    ...
    def _build_chart_permission_query(self, ptype):
        DocShare = trivena.qb.DocType("DocShare")
        Chart = trivena.qb.DocType("Insights Chart v3")
        DashboardChart = trivena.qb.DocType("Insights Dashboard Chart v3")

        OwnedCharts = trivena.qb.from_(Chart).select(Chart.name).where(Chart.owner == self.user)
    ...
    def _build_query_permission_query(self, ptype):
        ...
        OwnedQueries = trivena.qb.from_(Query).select(Query.name).where(Query.owner == self.user)

        AllowedWorkbooks = self._build_workbook_permission_query(ptype)

        LinkedWithAllowedWorkbooks = (
            trivena.qb.from_(Query)
            .select(Query.name)
            .left_join(AllowedWorkbooks)
    ...
    def _build_alert_permission_query(self, ptype):
        Alert = trivena.qb.DocType("Insights Alert")

        OwnedAlerts = trivena.qb.from_(Alert).select(Alert.name).where(Alert.owner == self.user)

        QueryWithWriteAccess = self._build_query_permission_query(ptype)

        LinkedWithQueryWithWriteAccess = (
            trivena.qb.from_(Alert)
            .select(Alert.name)
    ...
    def _build_resource_query(self, doctype):
        Resource = trivena.qb.DocType("Insights Resource Permission")
        ...
        return trivena.qb.from_(Resource).select(Resource.resource_name.as_("name")).where(condition)
