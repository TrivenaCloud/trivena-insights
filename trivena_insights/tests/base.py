...
from trivena_insights.tests.factories import as_user, is_visible
...
class InsightsIntegrationTestCase(IntegrationTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.original_user = trivena.session.user
        self.addCleanup(trivena.set_user, self.original_user)
        trivena.set_user("Administrator")
        self.before_test()
        if self.SAVEPOINT:
            trivena.db.savepoint(self.SAVEPOINT)
    ...
    def tearDown(self):
        try:
            trivena.set_user("Administrator")
            if self.SAVEPOINT:
                trivena.db.rollback(save_point=self.SAVEPOINT)
            self.after_test()
