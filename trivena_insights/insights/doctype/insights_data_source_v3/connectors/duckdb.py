...
def local_duckdb_write_lock(
    ...
    lock_name = f"insights_duckdb_write_{trivena.scrub(os.path.basename(path))}"
...
def local_duckdb_write_connection(
    ...
    2. Evicting and disconnecting any cached read-only connection for cache_key
       from trivena_insights.db_connections.
...
def get_duckdb_connection(data_source) -> DuckDBBackend:
    name = data_source.name or trivena.scrub(data_source.title)
    db_name = data_source.database_name
...
def get_http_secret(data_source, name, db_name):
    ...
    try:
        ...
        secret_name = f"http_auth_{trivena.scrub(name)}"
        scope_escaped = scope.replace("'", "''")

        headers = trivena.parse_json(headers) if isinstance(headers, str) else headers
        ...
        trivena.log_error(title="Error creating HTTP Secret for DuckDB", message=str(e))
        return
