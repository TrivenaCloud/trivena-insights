...
from trivena_framework.utils import add_days, nowdate

from trivena_insights.insights.doctype.insights_data_source_v3.ibis_utils import CircularQueryReferenceError
...
class TestQuerying(InsightsIntegrationTestCase):
    ...
    def delete_test_todos():
        todo_names = trivena.get_all(
            "ToDo",
            filters={"description": ["like", f"{TODO_PREFIX}%"]},
            pluck="name",
        )
        for todo_name in todo_names:
            trivena.delete_doc("ToDo", todo_name, force=True)
    ...
    def seed_todos(self):
        ...
        for record in records:
            todo = trivena.get_doc({"doctype": "ToDo", **record}).insert(ignore_permissions=True)
    ...
    def get_query(self, query_name):
        return trivena.get_doc(DT.QUERY, query_name)

    def test_builder_query_executes_from_source_operation(self):
        data_sources = set(trivena.get_all("Insights Data Source v3", pluck="name"))
