...
def create_user(
    ...
    **kwargs,
    ...
    if trivena.db.exists(DT.USER, email):
        user = trivena.get_doc(DT.USER, email)
    else:
    ...
    for role in roles or []:
        if not trivena.db.exists("Has Role", {"parent": email, "role": role}):
            user.add_roles(role)

    return trivena.get_doc(DT.USER, email)
...
def delete_users(*emails):
    for email in emails:
        trivena.delete_doc(DT.USER, email, force=True)
...
def create_test_workbook(owner, title=TEST_WORKBOOK_TITLE):
    with as_user(owner):
        return trivena.get_doc({"doctype": DT.WORKBOOK, "title": title}).insert()


def create_test_query(owner, workbook, title=TEST_QUERY_TITLE, operations=None):
    with as_user(owner):
        return trivena.get_doc(
            {
                "doctype": DT.QUERY,
                "title": title,
                "workbook": workbook,
...
def create_test_chart(owner, workbook, query=None, title=TEST_CHART_TITLE):
    ...
    return trivena.get_doc(DT.CHART, chart.name)
...
def create_test_dashboard(owner, workbook, chart=None, title=TEST_DASHBOARD_TITLE):
    ...
    return trivena.get_doc(DT.DASHBOARD, dashboard.name)


def execute_test_query(query_name):
    query = trivena.get_doc(DT.QUERY, query_name)
    with db_connections():
        return query.execute()


def is_visible(doctype, name):
    return bool(trivena.get_list(doctype, filters={"name": name}, pluck="name", limit=1))
...
def delete_workbooks(title_prefix=None, owners=None):
    ...
    workbooks = trivena.get_all(DT.WORKBOOK, filters=filters, pluck="name")
    for workbook in workbooks:
        trivena.delete_doc(DT.WORKBOOK, workbook, force=True)
