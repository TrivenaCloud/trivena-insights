...
import sqlparse
from trivena_framework.utils.data import flt
from trivena_framework.utils.safe_exec import safe_eval, safe_exec
from ibis import _
...
from trivena_insights import create_toast
from trivena_insights.cache_utils import make_digest
from trivena_insights.insights.doctype.insights_data_source_v3.data_warehouse import is_warehouse
from trivena_insights.insights.doctype.insights_table_v3.insights_table_v3 import (
    InsightsTablev3,
)
from trivena_insights.insights.query_builders.sql_functions import handle_timespan
from trivena_insights.insights.query_utils import extract_sql_table_refs
from trivena_insights.utils import create_execution_log
from trivena_insights.utils import deep_convert_dict_to_dict as _dict

from .ibis.functions import fiscal_year_start, week_start
from .ibis.utils import get_functions

try:
    from trivena_framework.concurrency_limiter import concurrent_limit
except ImportError:
...
class CircularQueryReferenceError(trivena.ValidationError):
...
class IbisQueryBuilder:
    ...
    def set_operations(self):
        operations = trivena.parse_json(self.operations)
        adhoc_filters_by_query = getattr(trivena.local, "insights_adhoc_filters", None) or {}
    ...
    def build(self) -> IbisQuery:
        if not hasattr(trivena.local, "_insights_building_queries"):
            trivena.local._insights_building_queries = set()

        if self.doc.name in trivena.local._insights_building_queries:
            raise CircularQueryReferenceError(
                trivena._('Circular query reference detected while building "{0}"').format(self.title)
            )

        trivena.local._insights_building_queries.add(self.doc.name)
        try:
            self.query = None
            for idx, operation in enumerate(self.operations):
                try:
                    ...
                    operation_type_title = trivena.bold(operation.type.title())
                    create_toast(
            ...
            trivena.local._insights_building_queries.discard(self.doc.name)
    ...
    def get_table_or_query(self, table_args):
        ...
        if table_args.type == "query":
            q = trivena.get_doc("Insights Query v3", table_args.query_name)
            _table = q.build(use_live_connection=self.use_live_connection)

        if _table is None:
            trivena.throw("Table or Query not found")
    ...
    def get_column(self, column_name, throw=True):
        ...
        if throw:
            trivena.throw(f"Column {column_name} does not exist in the table")
    ...
    def translate_join_condition(self, join_args, right_table):
        def left_eq_right_condition(left_column, right_column):
            ...
            trivena.throw("Join condition is not valid")
    ...
    def rename_duplicate_columns(self, right_table):
        ...
        def get_new_name(col):
            ...
            while is_conflicting(f"{new_name}_{n}"):
                n += 1
                if n > 20:
                    trivena.throw("Too many duplicate columns")
    ...
    def make_filter_condition(self, filter_args):
        ...
        if operator_fn is None:
            trivena.throw(f"Operator {filter_operator} is not supported")
    ...
    def apply_filter_group(self, filter_group_args):
        ...
        trivena.throw(f"Logical operator {logical_operator} is not supported")
    ...
    def apply_sql(self, sql_args):
        ...
        ds = trivena.get_doc("Insights Data Source v3", data_source)
        ...
        check_permissions = trivena.db.get_single_value(
            "Insights Settings", "enable_permissions"
        ) or trivena.db.get_single_value("Insights Settings", "apply_user_permissions")
    ...
    def apply_code(self, code_args):
        ...
        adhoc_filters = trivena.as_json(getattr(trivena.local, "insights_adhoc_filters", {}))
    ...
    def apply_aggregate(self, column, aggregate_function):
        ...
        trivena.throw(f"Aggregate function {aggregate_function} is not supported")
    ...
    def apply_granularity(self, column, granularity, data_type=None):
        ...
        if granularity not in truncate_unit:
            trivena.throw(f"Granularity {granularity} is not supported")
    ...
    def apply_time_granularity(self, column, granularity):
        ...
        trivena.throw(f"Granularity {granularity} is not supported for Time columns")

    def evaluate_expression(self, expression, additonal_context=None):
        if not expression or not expression.strip():
            raise ValueError(f"Invalid expression: {expression}")

        trivena.flags.current_ibis_query = self.query
        context = trivena._dict()
        context.pandas = trivena._dict()
        ...
        trivena.flags.current_ibis_query = None
        return ret
...
def cache_results(cache_key, result: pd.DataFrame, cache_expiry=3600):
    ...
    data = trivena.as_json(data)
    trivena.cache().set_value(cache_key, data, expires_in_sec=cache_expiry)


def get_cached_results(cache_key) -> pd.DataFrame:
    cache_key = "insights:query_results:" + cache_key
    data = trivena.cache().get_value(cache_key)
    if not data:
        return None
...
def has_cached_results(cache_key):
    cache_key = "insights:query_results:" + cache_key
    return trivena.cache().get_value(cache_key) is not None
...
def get_code_results(code: str, variables=None):
    ...
    if not isinstance(results, pd.DataFrame):
        try:
            ...
            results = pd.DataFrame(_json.loads(trivena.as_json(results)))
...
@contextmanager
def ensure_rollback():
    hash = trivena.generate_hash(length=4)
    try:
        trivena.db.savepoint(f"save_point_{hash}")
        yield
    finally:
        trivena.db.rollback(save_point=f"save_point_{hash}")
