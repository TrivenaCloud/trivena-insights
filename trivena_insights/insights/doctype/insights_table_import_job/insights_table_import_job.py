...
from trivena_framework.utils import get_datetime, now, now_datetime
from trivena_framework.utils.password import get_decrypted_password
from trivena_framework.utils.safe_exec import safe_exec

from trivena_insights.insights.doctype.insights_data_source_v3.data_warehouse import WarehouseTableWriter
from trivena_insights.insights.doctype.insights_data_source_v3.ibis_utils import SafePandasDataFrame
...
class InsightsTableImportJob(Document):
    ...
    if TYPE_CHECKING:
        ...
        from trivena_insights.insights.doctype.insights_secret_key.insights_secret_key import InsightsSecretKey
    ...
    def validate_cron_expression(self):
        ...
        if self.schedule:
            try:
                ...
                trivena.throw(f"Invalid cron expression: {e!s}")

    def on_update(self):
        self.table_name = trivena.scrub(self.table_name)
    ...
    def enqueue(self):
        ...
        trivena.msgprint(f"Job '{self.title}' has been queued for execution.")
    ...
    def bulk_enqueue(self, run_count: int):
        ...
        if run_count < 1 or run_count > 100:
            trivena.throw("Run count must be between 1 and 100")

        bulk_enqueue_runs(self.name, run_count)
        trivena.msgprint(f"Queued {run_count} runs for '{self.title}'")
...
class TableImportJobRun:
    def __init__(self, job_name: str):
        self.job = trivena.get_doc("Insights Table Import Job", job_name)
    ...
    def _mark_failure(self, error: Exception):
        try:
            ...
            traceback = trivena.as_unicode(trivena.get_traceback(with_context=True))
...
class JobState:
    ...
    def _load_state(self) -> dict:
        ...
        try:
            ...
            trivena.log_error(title=f"Invalid state JSON for job {self._job.name}")
            return {}
...
def run_scheduled_imports():
    ...
    for job_data in jobs:
        try:
            job = trivena.get_doc("Insights Table Import Job", job_data.name)
