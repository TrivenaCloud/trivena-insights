...
import trivena_framework.utils
...
from trivena_insights.utils import deep_convert_dict_to_dict
...
class InsightsWorkbook(Document):
    ...
    def before_save(self):
        self.title = self.title or f"Workbook {trivena.utils.cint(self.name)}"

    def on_trash(self):
        try:
            backup_data = trivena.as_json(self.export())
            self.db_set("data_backup", backup_data)
        except Exception as e:
            trivena.log_error(f"Failed to backup workbook {self.name}: {e!s}")

        for q in trivena.get_all("Insights Query v3", {"workbook": self.name}):
            trivena.delete_doc("Insights Query v3", q.name, force=True, ignore_permissions=True)
        for c in trivena.get_all("Insights Chart v3", {"workbook": self.name}):
            trivena.delete_doc("Insights Chart v3", c.name, force=True, ignore_permissions=True)
        for d in trivena.get_all("Insights Dashboard v3", {"workbook": self.name}):
            trivena.delete_doc("Insights Dashboard v3", d.name, force=True, ignore_permissions=True)
        for f in trivena.get_all("Insights Folder", {"workbook": self.name}):
            trivena.delete_doc("Insights Folder", f.name, force=True, ignore_permissions=True)

    def after_insert(self):
        capture("workbook_created", "trivena_insights")
        ...
        # Parse backup data
        backup = trivena.parse_json(self.data_backup)
    ...
    def restore_workbook_contents(self, workbook_data, target_workbook_name, ignore_permissions=False):
        ...
        for folder in workbook_data.get("dependencies", {}).get("folders", []):
            ...
            new_folder = trivena.new_doc("Insights Folder")
        ...
        for name, query in workbook_data.get("dependencies", {}).get("queries", {}).items():
            ...
            new_query = trivena.new_doc("Insights Query v3")
        ...
        for name, _ in workbook_data.get("dependencies", {}).get("queries", {}).items():
            new_query = trivena.get_doc("Insights Query v3", id_map[name])
            operations = deep_convert_dict_to_dict(trivena.parse_json(new_query.operations))
        ...
        for _, dashboard in workbook_data.get("dependencies", {}).get("dashboards", {}).items():
            ...
            new_dashboard = trivena.new_doc("Insights Dashboard v3")
            new_dashboard.update(dashboard)
            new_dashboard.workbook = target_workbook_name

            items = deep_convert_dict_to_dict(trivena.parse_json(dashboard["items"]))
            for item in items:
    ...
    def as_dict(self, *args, **kwargs):
        ...
        chart_queries = trivena.get_all(
            "Insights Chart v3",
            filters={"workbook": self.name},
            pluck="data_query",
        )
        d.queries = trivena.get_all(
            "Insights Query v3",
            filters={
                "workbook": self.name,
        ...
        d.dashboards = trivena.get_all(
            "Insights Dashboard v3",
            filters={"workbook": self.name},
            fields=["name", "title"],
            order_by="creation asc",
        )
        d.folders = trivena.as_json(d.folders)
        d.queries = trivena.as_json(d.queries)
        d.charts = trivena.as_json(d.charts)
        d.dashboards = trivena.as_json(d.dashboards)
    ...
    def export(self):
        ...
        chart_queries = trivena.get_all("Insights Chart v3", {"workbook": self.name}, pluck="data_query")
        queries = trivena.get_all(
            "Insights Query v3",
            filters={
                "workbook": self.name,
                "name": ["not in", chart_queries],
        ...
        for q in queries:
            q.operations = trivena.parse_json(q.operations)
            workbook["dependencies"]["queries"][q.name] = q

        charts = trivena.get_all(
            "Insights Chart v3",
            filters={"workbook": self.name},
            fields=[
                "name",
        ...
        for c in charts:
            c.config = trivena.parse_json(c.config)
        ...
        if active_folders:
            workbook["dependencies"]["folders"] = trivena.get_all(
                "Insights Folder",
        ...
        for d in dashboards:
            d["items"] = trivena.parse_json(d["items"])
    ...
    def import_query(self, query: dict | str):
        from trivena_insights.insights.doctype.insights_query_v3.insights_query_v3 import import_query

        return import_query(query, self.name)

    @trivena.whitelist()
    def import_chart(self, chart: dict | str):
        from trivena_insights.insights.doctype.insights_chart_v3.insights_chart_v3 import import_chart
    ...
    def get_lineage_graph(self):
        ...
        trivena.only_for("Insights Admin")

        Ref = trivena.qb.DocType("Insights Query Reference")
        Query = trivena.qb.DocType("Insights Query v3")

        edges = (
            trivena.qb.from_(Ref)
            .join(Query)
...
def import_workbook(workbook):
    workbook = trivena.parse_json(workbook)
    workbook = deep_convert_dict_to_dict(workbook)

    # Create a new Insights Workbook
    new_workbook = trivena.new_doc("Insights Workbook")
