...
def execute():
    ...
    leaked into the UI and broke the table -> doctype mapping for trivena databases, so
    ...
    for source in sources:
        ...
        try:
            ...
            trivena.db.rollback()
            trivena.log_error(title=f"Failed to strip schema prefix for {source.name}")
...
def rename_tables(data_source, prefix) -> dict[str, str]:
    ...
    for row in rows:
        ...
        if trivena.db.exists("Insights Table v3", new_name):
            # an unqualified record already exists — drop the duplicate
            discard_table(data_source, row)
            renames[row.table] = new_table
            continue

        # renaming by hand rather than through `trivena.rename_doc`: every referrer is
        ...
        if row.stored and not rename_warehouse_table(data_source, row.table, new_table):
            ...
            trivena.db.set_value("Insights Table v3", new_name, "stored", 0, update_modified=False)
...
def discard_table(data_source, row):
    ...
    trivena.delete_doc("Insights Table v3", row.name, force=True, ignore_permissions=True)


def rename_warehouse_table(data_source, old_table, new_table) -> bool:
    old_name = trivena.scrub(old_table)
    new_name = trivena.scrub(new_table)
...
def update_queries(data_source, renames):
    ...
    for query in queries:
        operations = trivena.parse_json(query.operations)
        if not operations:
            continue
