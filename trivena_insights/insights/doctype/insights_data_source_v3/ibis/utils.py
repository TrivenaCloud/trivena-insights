...
from trivena_framework.utils.safe_exec import SERVER_SCRIPT_FILE_PREFIX, safe_exec
from ibis import selectors as s
from jedi import Script


def get_functions():
    import trivena_insights.insights.doctype.insights_data_source_v3.ibis.functions as functions
...
def get_code_completions(code: str, column_options: str | None = None):
    import_statement = """from trivena_insights.insights.doctype.insights_data_source_v3.ibis.functions import *\nfrom ibis import selectors as s"""
...
def parse_column_metadata(column_options: str):
    columns = trivena.parse_json(column_options) or []
...
def validate_types(expression: str, columns: list[dict]):
    ...
    try:
        ...
        _, _, tb = sys.exc_info()
        line = get_error_line(tb)
        trivena.log_error(f"Unexpected validation error: {e!s}")
