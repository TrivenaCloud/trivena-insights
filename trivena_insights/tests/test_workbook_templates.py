...
from trivena_insights.api.templates import (
...
from trivena_insights.insights.doctype.insights_workbook.insights_workbook import (
    InsightsWorkbook,
    import_workbook,
)
from trivena_insights.tests.base import InsightsIntegrationTestCase
from trivena_insights.tests.factories import USER_1, create_test_user, delete_users
from trivena_insights.tests.workbook_utils import get_workbook
...
APPS_WITHOUT_ERPNEXT = ["frappe", "trivena_insights"]
APPS_WITH_ERPNEXT = ["frappe", "trivena_insights", "erpnext"]


def installed_apps(apps):
    # patch our narrow seam, not the global — patching trivena.get_installed_apps
...
def cleanup_template_workbooks():
    ...
    for name in trivena.get_all("Insights Workbook", filters={"from_template": ["!=", ""]}, pluck="name"):
        trivena.delete_doc("Insights Workbook", name, force=True)
...
class TestWorkbookTemplates(InsightsIntegrationTestCase):
    ...
    def test_import_is_shared_site_wide_and_marks_template(self):
        ...
        self.assertEqual(
            trivena.db.get_value("Insights Workbook", workbook_name, "from_template"),
            TEMPLATE,
        )
        ...
        trivena.delete_doc("Insights Workbook", workbook_name, force=True)
        with self.as_user(USER_2), installed_apps(APPS_WITH_ERPNEXT):
            sales = {t["name"]: t for t in get_workbook_templates()}[TEMPLATE]
            self.assertIsNone(sales["imported_workbook"])
    ...
    def test_imported_workbook_owned_by_administrator_and_org_shared(self):
        ...
        self.assertEqual(trivena.db.get_value("Insights Workbook", workbook_name, "owner"), "Administrator")
        child_owners = trivena.get_all("Insights Chart v3", filters={"workbook": workbook_name}, pluck="owner")
    ...
    def test_non_admin_can_read_but_not_write_shared_copy(self):
        ...
        with self.as_user(USER_2):
            self.assertFalse(
                trivena.has_permission("Insights Workbook", ptype="write", doc=str(workbook_name))
            )
    ...
    def test_double_import_returns_existing_without_duplicating(self):
        ...
        self.assertEqual(trivena.db.count("Insights Workbook", {"from_template": TEMPLATE}), 1)
    ...
    def test_non_admin_cannot_import(self):
        ...
        self.assertEqual(trivena.db.count("Insights Workbook", {"from_template": TEMPLATE}), 0)

    def test_create_blocked_when_required_apps_missing(self):
        with self.as_user(ADMIN_USER), installed_apps(APPS_WITHOUT_ERPNEXT):
            with self.assertRaises(trivena.ValidationError):
                create_workbook_from_template(TEMPLATE)
    ...
    def test_create_workbook_from_template_round_trips(self):
        ...
        items = trivena.parse_json(trivena.db.get_value("Insights Dashboard v3", dashboard_name, "items"))
    ...
    def test_import_records_version_and_is_pristine(self):
        ...
        self.assertEqual(
            trivena.db.get_value("Insights Workbook", workbook_name, "imported_version"), TEMPLATE_VERSION
        )
    ...
    def test_update_replaces_contents_in_place(self):
        with self.as_user(ADMIN_USER), installed_apps(APPS_WITH_ERPNEXT):
            workbook_name = create_workbook_from_template(TEMPLATE)["workbook"]
            chart = trivena.get_all("Insights Chart v3", {"workbook": workbook_name}, pluck="name")[0]
            # a local edit that the update must roll back to the shipped definition
            trivena.db.set_value("Insights Chart v3", chart, "title", "Edited")
        ...
        self.assertEqual(
            trivena.db.get_value("Insights Workbook", workbook_name, "imported_version"), NEXT_VERSION
        )
        self.assertFalse(trivena.db.exists("Insights Chart v3", chart))
    ...
    def test_migrate_sync_leaves_customized_copy_for_manual_update(self):
        with self.as_user(ADMIN_USER), installed_apps(APPS_WITH_ERPNEXT):
            workbook_name = create_workbook_from_template(TEMPLATE)["workbook"]
            chart = trivena.get_all("Insights Chart v3", {"workbook": workbook_name}, pluck="name")[0]
            trivena.db.set_value("Insights Chart v3", chart, "title", "Locally Edited")
        ...
        self.assertEqual(trivena.db.get_value("Insights Chart v3", chart, "title"), "Locally Edited")
    ...
    def test_migrate_sync_rolls_back_a_failed_update(self):
        with self.as_user(ADMIN_USER), installed_apps(APPS_WITH_ERPNEXT):
            workbook_name = create_workbook_from_template(TEMPLATE)["workbook"]
        charts_before = trivena.get_all("Insights Chart v3", {"workbook": workbook_name}, pluck="name")
        ...
        charts_after = trivena.get_all("Insights Chart v3", {"workbook": workbook_name}, pluck="name")
    ...
    def test_every_committed_template_is_valid_and_importable(self):
        ...
        for name in template_names:
            with self.subTest(template=name):
                ...
                self.assertTrue(trivena.db.exists("Insights Workbook", imported_name))
                trivena.delete_doc("Insights Workbook", imported_name, force=True)
