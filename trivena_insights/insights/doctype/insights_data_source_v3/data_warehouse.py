...
from trivena_framework.query_builder.functions import IfNull, Max, Min
from trivena_framework.utils import add_days, get_datetime, get_files_path, now, now_datetime
from trivena_framework.utils.background_jobs import is_job_enqueued
from ibis import _
...
from trivena_insights.insights.doctype.insights_data_source_v3.connectors.duckdb import (
    local_duckdb_write_connection,
    local_duckdb_write_lock,
    open_local_duckdb,
)
from trivena_insights.utils import InsightsDataSourcev3, InsightsTablev3

WAREHOUSE_DB_NAME = "trivena_insights"
...
class WarehouseTable:
    def __init__(self, data_source: str, table_name: str):
        ...
        self.warehouse_table_name = trivena.scrub(table_name)
    ...
    def validate(self):
        if not self.data_source:
            trivena.throw("Data Source is required.")
        if not self.table_name:
            trivena.throw("Table Name is required.")
    ...
    def get_ibis_table(self, import_if_not_exists: bool = True) -> Expr:
        try:
            ...
            trivena.log_error(e)
            trivena.throw("Error accessing the data warehouse. Please try again.")
    ...
    def enqueue_import(self):
        if trivena.db.get_value("Insights Data Source v3", self.data_source, "type") == "REST API":
            trivena.throw("Import not supported for API data sources")
...
class WarehouseTableImporter:
    ...
    def enqueue_import(self):
        job_id = f"import_{trivena.scrub(self.table.data_source)}_{trivena.scrub(self.table.table_name)}"

        if is_job_enqueued(job_id) or self.import_in_progress():
            insights.create_toast(
                f"Import for {trivena.bold(self.table.table_name)} is in progress."
        ...
        with db_connections():
            ...
            try:
                ...
                if self.log.status != "Failed":
                    ...
                    self._log(trivena.get_traceback())
                raise
            finally:
                self.update_log()

        insights.create_toast(
            f"Imported {trivena.bold(self.table.table_name)} to the data store. "
    ...
    def prepare_log(self):
        self.log = trivena.new_doc("Insights Table Import Log")
        self.log.db_insert()
        self.log.db_set(
            {
                "data_source": self.table.data_source,
                "table_name": self.table.table_name,
                "started_at": trivena.utils.now(),
                "status": "In Progress",
            },
            commit=True,
        )
        ...
        insights.create_toast(
            f"Importing {trivena.bold(self.table.table_name)} to the data store. "
            "You may not see the results till the import is completed.",
    ...
    def prepare_settings(self) -> dict:
        ...
        self.settings.row_limit = (
            table_doc.row_limit
            or trivena.db.get_single_value("Insights Settings", "max_records_to_sync")
            or 10_00_000
        )
        self.settings.before_import_script = table_doc.before_import_script or ""
        self.settings.memory_limit = (
            trivena.db.get_single_value("Insights Settings", "max_memory_usage") or 512
        )
    ...
    def update_log(self):
        ended_at = trivena.utils.now()
        self.log.db_set(
            {
                "ended_at": ended_at,
                "time_taken": trivena.utils.time_diff_in_seconds(ended_at, self.log.started_at),
            },
            commit=True,
        )
    ...
    def update_insights_table(self):
        ...
        t.stored = 1
        t.last_synced_on = trivena.utils.now()
...
def enqueue_warehouse_table_import(data_source: str, table_name: str):
    job_id = f"import_{trivena.scrub(data_source)}_{trivena.scrub(table_name)}"
    trivena.enqueue(
...
def get_warehouse_schema_name(data_source: str) -> str:
    ...
    return trivena.scrub(data_source).replace(".", "_")
...
def prune_unused_tables(cutoff) -> list[str]:
    ...
    oldest_execution = trivena.db.get_value(
...
def get_last_execution_per_table() -> dict[tuple[str, str], object]:
    ...
    logged separately.
    """
    ExecutionLog = trivena.qb.DocType("Insights Query Execution Log")
    executions = (
        trivena.qb.from_(ExecutionLog)
...
def get_first_import_per_table() -> dict[tuple[str, str], object]:
    ImportLog = trivena.qb.DocType("Insights Table Import Log")
    imports = (
        trivena.qb.from_(ImportLog)
        .select(
...
def drop_orphan_warehouse_tables() -> list[str]:
    ...
    expected = {
        (get_warehouse_schema_name(t.data_source), trivena.scrub(t.table))
        for t in trivena.get_all("Insights Table v3", filters={"stored": 1}, fields=["data_source", "table"])
    }
