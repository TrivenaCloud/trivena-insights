...
def extract_sql_table_refs(raw_sql: str, dialect: sg.Dialect | None = None) -> list[trivena._dict]:
    try:
...
def extract_table_deps_from_sql_operations(operations: list) -> list[dict]:
    ...
    for op in operations:
        ...
        if not raw_sql or not ds:
            continue
        db_type = trivena.db.get_value("Insights Data Source v3", ds, "database_type", cache=True)
...
def sync_query_references(query_name: str, operations) -> None:
    ...
    """
    from trivena_framework.model.document import bulk_insert

    ops = trivena.parse_json(operations) or []
    ...
    for tbl in all_table_deps:
        ref = trivena.new_doc("Insights Query Reference")
        ref.name = trivena.generate_hash(length=10)
    ...
    trivena.db.delete("Insights Query Reference", {"query": query_name})
    if docs:
