...
def make_data_source(data_source):
    data_source = trivena._dict(data_source)
    ds = trivena.new_doc("Insights Data Source v3")
...
def get_data_sources_of_tables(table_names: list[str]):
    ...
    if not isinstance(table_names, list):
        trivena.throw("Table names should be a list")
    if not all(isinstance(table_name, str) for table_name in table_names):
        trivena.throw("Table names should be a list of strings")
